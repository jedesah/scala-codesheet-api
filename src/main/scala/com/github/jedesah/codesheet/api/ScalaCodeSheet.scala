...
object ScalaCodeSheet {
    ...
    def evaluate(AST: Tree, outputResult: List[String], toolBox: ToolBox[reflect.runtime.universe.type], symbols: Set[Tree] = Set()): (List[String], Set[Tree]) = {
      ...
      AST match {
        ...
        case defdef : DefDef => {
            val sampleValues = defdef.vparamss.flatten.zip(sampleIntValues).map{
              case (valDef, primeNumber) => ValDef(Modifiers(), valDef.name, TypeTree(), Literal(Constant(primeNumber)))
            }
