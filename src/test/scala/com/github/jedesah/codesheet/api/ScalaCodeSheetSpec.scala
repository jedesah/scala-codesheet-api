...
class ScalaCodeSheetSpec extends Specification {
	...
	"ScalaCodeSheet" should {
		"expressions" in {
			"literal" in {
				computeResults("1") ==== StandardResult(List(SimpleExpressionResult(1, line = 1)), "")
			}
			"typical" in {
				computeResults("1 + 1") ==== StandardResult(List(SimpleExpressionResult(2, line = 1)), "")
			}
			"two lines" in {
				val code = """1 + 1
                             |4 * 4""".stripMargin
				computeResults(code) ==== StandardResult(List(SimpleExpressionResult(2, line = 1), SimpleExpressionResult(16, line = 2)), "")
			}
			"with empty line" in {
				val code = """1 + 1
                             |
                             |4 * 4""".stripMargin
				computeResults(code) must beLike { case StandardResult(List(line1, line3), "") =>
			...
			"expression returning Unit" in {
				...
				computeResults(code) must beLike { case StandardResult(List(IfThenElseResult(cond, executed, 1)), "") =>
			...
			"empty" in {
				computeResults("") ==== StandardResult(Nil, "")
			}
			"string interpolation" in {
				"steps" in {
					computeResults(""" s"allo" """) ==== StandardResult(List(SimpleExpressionResult("allo", line = 1)), "")
				}
				"no steps" in {
					computeResults(""" s"allo" """, enableSteps = false) ==== StandardResult(List(SimpleExpressionResult("allo", line = 1)), "")
				}
			}
		}
		"function definition" in {
			"simple definition" in {
				...
				computeResults(code) ==== StandardResult(List(DefDefResult("hello", Nil, None , inner, line = 1)), "")
			}
			"complex definition" in {
				val code = "def foo = 10 * 4 - 2"
				val inner = SimpleExpressionResult(38, line = 1)
				computeResults(code) ==== StandardResult(List(DefDefResult("foo", Nil, None , inner, line = 1)), "")
			}
			"with use" in {
				val code = """def hello = "Hello!"
							|
				...
				computeResults(code) ==== StandardResult(List(first, second), "")
			}
			"overlapping scope" in {
				val code = """val a = 4
							 |def tot(a: String) = a * 2""".stripMargin
				computeResults(code) must beLike { case StandardResult(List(first, second), "") =>
			...
			"with params" in {
				"basic types" in {
					"Int" in {
						...
						computeResults(code) must beLike { case StandardResult(List(first), "") =>
					...
					"Any" in {
						...
						computeResults(code, enableSteps =  false) must beLike { case StandardResult(List(first), "") =>
							first must beLike { case DefDefResult("foo", params, None, rhs, 1) =>
					...
					"case" in {
						"one case class definition" in {
							...
							"two param occurence" in {
								...
								computeResults(code, enableSteps = false) must beLike { case StandardResult(List(first, second), "") =>
									first must beLike {case ClassDefResult("Car", params, BlockResult(Nil, 1), 1) =>
										params must beLike { case List(a,b) =>
											structureEquals(a, AssignOrNamedArg(Ident(newTermName("year")), Literal(Constant(3))))
											structureEquals(b, AssignOrNamedArg(Ident(newTermName("model")), Literal(Constant("foo"))))
						...
						"two case class definitions" in {
							"use of first one" in {
								...
								computeResults(code, enableSteps = false) must beLike { case StandardResult(List(first, second, third), "") =>
									first must beLike { case ClassDefResult("Car", params, BlockResult(Nil, 1), 1) =>
										params must beLike { case List(a,b) =>
											structureEquals(a, AssignOrNamedArg(Ident(newTermName("year")), Literal(Constant(3))))
											structureEquals(b, AssignOrNamedArg(Ident(newTermName("model")), Literal(Constant("foo"))))
						...
						"multiple case class definitions" in {
							"one param occurence" in {
								"case 1" in {
									...
									computeResults(code, enableSteps = false) must beLike { case StandardResult(List(first, second, third, fourth), "") =>
										first must beLike {case ClassDefResult("Car", params, BlockResult(Nil, 1), 1) =>
											params must beLike { case List(a,b) =>
												structureEquals(a, AssignOrNamedArg(Ident(newTermName("year")), Literal(Constant(3))))
												structureEquals(b, AssignOrNamedArg(Ident(newTermName("model")), Literal(Constant("foo"))))
						...
						"multiple function definitions" in {
							val code = """case class Car(year: Int, model: String)
									...
									computeResults(code, enableSteps = false) must beLike { case StandardResult(List(first, second, third, fourth, fifth), "") =>
										first must beLike {case ClassDefResult("Car", params, BlockResult(Nil, 1), 1) =>
											params must beLike { case List(a,b) =>
												structureEquals(a, AssignOrNamedArg(Ident(newTermName("year")), Literal(Constant(3))))
												structureEquals(b, AssignOrNamedArg(Ident(newTermName("model")), Literal(Constant("foo"))))
				...
				"multiline" in {
					...
					"multiple inner value definition" in {
						val code = """def foo(a: List[Int], b: Int) = {
                                     ...
                                     |}""".stripMargin
						computeResults(code, enableSteps = false) must beLike { case StandardResult(List(defdef), "") =>
		...
		"variable definition" in {
			"simple" in {
				val code = "var a = 56"
				computeResults(code) must beLike { case StandardResult(List(only), "") =>
		...
		"block" in {
			val code = """{
                         ...
                         |	5
                         |}""".stripMargin
			computeResults(code) must beLike { case StandardResult(List(block), "") =>
		...
		"class definition" in {
			"simple" in {
				...
				computeResults(code, enableSteps = false) must beLike { case StandardResult(List(classDef), "") =>
			...
			"with instantiation" in {
				...
				computeResults(code, enableSteps = false) must beLike { case StandardResult(List(classDef, valDef), "") =>
				 	classDef must beLike {case ClassDefResult("Car", params, BlockResult(Nil, 1),1) =>
						params must beLike { case List(a, b) => 
							structureEquals(a, AssignOrNamedArg(Ident(newTermName("model")), Literal(Constant("foo"))))
							structureEquals(b, AssignOrNamedArg(Ident(newTermName("year")), Literal(Constant(3))))
			...
			"with use" in {
				...
				computeResults(code, enableSteps = false) must beLike { case StandardResult(List(classDef, valDef, expression1, expression2), "") =>
				 	classDef must beLike {case ClassDefResult("Car", params, BlockResult(Nil, 1),1) =>
						params must beLike { case List(a, b) => 
							structureEquals(a, AssignOrNamedArg(Ident(newTermName("model")), Literal(Constant("foo"))))
							structureEquals(b, AssignOrNamedArg(Ident(newTermName("year")), Literal(Constant(3))))
			...
			"complex" in {
				"1" in {
					val code = """case class Car(model: String, year: Int) {
                                 ...
                                 |a.drive""".stripMargin
					computeResults(code, enableSteps = false) must beLike { case StandardResult(List(classDef, valDef, expression1), "") =>
		...
		"object definition" in {
			"with only simple values" in {
				val code = """object MySingleton {
							...
							|	val tretre = "goo"
							| }""".stripMargin
				computeResults(code, enableSteps = false) must beLike{ case StandardResult(List(moduleDef), "") =>
		...
		"sample generation" in {
			"abstract class" in {
				"simple hierarchy" in {
					...
					computeResults(code, enableSteps = false) must beLike { case StandardResult(List(classDef, cat, dog, gogo), "") =>
				...
				"no concrete class" in {
					"simple" in {
						...
						computeResults(code, enableSteps = false) must beLike { case StandardResult(List(classDef, gogogo), "") =>
							classDef ==== ClassDefResult("Animal", Nil, BlockResult(Nil, 1),1)
					...
					"with abstract members" in {
						"one value definition" in {
							...
							computeResults(code, enableSteps = false) must beLike { case StandardResult(List(classDef, gogogi), "") =>
								classDef ==== ClassDefResult("Animal", Nil, BlockResult(Nil, 1),1)
			...
			"trait" in {
				"simple hierarchy" in {
					...
					computeResults(code, enableSteps = false) must beLike { case StandardResult(List(cat, dog, tti), "") =>
						cat must beLike { case ClassDefResult("Cat", List(a), BlockResult(Nil, 2), 2) =>
							structureEquals(a, AssignOrNamedArg(Ident(newTermName("a")), Literal(Constant(3))))
						}
						dog must beLike { case ClassDefResult("Dog", List(b), BlockResult(Nil, 3), 3) =>
				...
				"no concrete class" in {
					"simple" in {
						...
						computeResults(code, false) must beLike { case StandardResult(List(tto), "") =>
					...
					"with abstract members" in {
						"one value definition" in {
							val code = """trait Animal {
                                         ...
                                         |}
                                         |def gogogi(yui: Animal) = yui.y + 45""".stripMargin
							computeResults(code, false) must beLike { case StandardResult(List(gogogi), "") =>
								gogogi must beLike { case DefDefResult("gogogi", List(yui), None, expr, 4) =>
		...
		"???" in {
			"simple definitions" in {
				"value definition" in {
					val code = "val a = ???"
					computeResults(code, false) must beLike { case StandardResult(List(ValDefResult("a", None, rhs, 1)), "") =>
				...
				"function definition" in {
					"no params" in {
						val code = "def gog = ???"
						computeResults(code, false) must beLike { case StandardResult(List(DefDefResult("gog", Nil, None, rhs, 1)), "") =>
							rhs ==== SimpleExpressionResult(NotImplementedResult, Nil, 1)
						}
					}
					"with params" in {
						val code = "def gogg(a: Int) = ???"
						computeResults(code, false) must beLike { case StandardResult(List(DefDefResult("gogg", params, None, rhs, 1)), "") =>
							params must beLike { case List(param) =>
			...
			"complex scenarios" in {
				"unimplemented value definition with later use" in {
					"1" in {
						val code = """val a: Int = ???
									 |val b = a""".stripMargin
						computeResults(code, false) must beLike { case StandardResult(List(a, b), "") =>
		...
		"throws an exception" in {
			...
			"value definition" in {
				val code = "val a = 10 / 0"
				computeResults(code, false) must beLike { case StandardResult(List(a), "") =>
			...
			"value definition followed by" in {
				"an unrelated expression" in {
					val code = """val a = 10 / 0
								 |7 * 7""".stripMargin
					computeResults(code, false) must beLike { case StandardResult(List(a, expr), "") =>
						a must beLike { case ValDefResult("a", None, SimpleExpressionResult(ExceptionValue(ex), Nil, 1), 1) =>
							verifyException(ex)
						}
			...
			"top level" in {
				"no output" in {
					"throws an exception" in {
						val code = "var a = 10 / 0"
						computeResults(code, false) must beLike { case ExceptionResult(_, "") => ok }
					}
					"throws the right exception" in {
						val code = "var a = 10 / 0"
						computeResults(code, false) must beLike { case ExceptionResult(ex, "") =>
							verifyException(ex)
						}
					}.pendingUntilFixed("Something weird is happenning here that is changing the type of the exception")
				}
				"output" in {
					val message = "JeanRemi works too hard"
					val code = s"""print("$message")
								 |var a = 10 / 0""".stripMargin
					computeResults(code, false) must beLike { case ExceptionResult(_, actualMessage) =>
		...
		"import" in {
			"simple" in {
				...
				computeResults(code, false) must beLike { case StandardResult(List(IfThenElseResult(_, executed, 2)), "") =>
			...
			"relative" in {
				val code = """import scala._
				             |import util.Random
				             |if (Random.nextBoolean) 10 else 10""".stripMargin
				computeResults(code, false) ==== StandardResult(List(SimpleExpressionResult(10, Nil, 3)), "")
		...
		"mutability" in {
			"println" in {
				val code = """object A {
							 ...
							 |}
							 |A.print""".stripMargin
				computeResults(code, false) must beLike { case StandardResult(List(module, expr), "Hello World!") =>
			...
			"variable" in {
				val code = """var a = 5
							 ...
							 |}
							 |a""".stripMargin
				computeResults(code, false) must beLike { case StandardResult(List(_, _, expr), "") =>
		...
		"ouput" in {
			...
			computeResults(code, false) must beLike { case StandardResult(List(print1, expr, print2), output) =>
		...
		"dependencies" in {
			...
			computeResults(code, false) must beLike { case StandardResult(List(expr), "") =>
